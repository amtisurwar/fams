...
class StartTimer extends Component {
  ...
  componentWillMount() {
    ...
    if (taskStatus == "Resumed" || taskStatus == "Started") {
      ...
      // let totalSec = this.getTimeInSec(new Date().getTime(), new Date(momentTz.tz(momentTz(checkInTime).tz('America/Los_Angeles'), "Asia/Kolkata").format()).getTime());
      let totalSec = this.getTimeInSec(new Date(momentTz().tz('Asia/Kolkata').format()).getTime(), new Date(momentTz.tz(momentTz(checkInTime).tz('America/Los_Angeles'), "Asia/Kolkata").format()).getTime());
  ...
  _handleAppStateChange = (nextAppState) => {
    ...
    if (this.state.appState.match(/inactive|background/) && nextAppState === 'active') {
      if (this.state.taskstatus == "Resumed" || this.state.taskstatus == "Started") {
        ...
        checkInTime = checkInTime.replace('-', '/');
        checkInTime = checkInTime.replace('T', ' ');
        let totalSec = this.getTimeInSec(new Date(momentTz().tz('Asia/Kolkata').format()).getTime(), new Date(this.state.timeElapsed).getTime());
        // let addedSec = this.getTimeInSec(new Date(checkInTime).getTime(), new Date(this.state.timeElapsed).getTime());
        let secCalculate = parseInt(totalSec, 10) + parseInt(this.state.totalworksec, 10) ;
  ...
  jobTaskTracking = (jobStatus) => {
    ...
    const dataValue = { "Task_ID": taskId, "User_ID": userId, "Job_ID": jobId, "TrackingDatetime": moment().format('YYYY-MM-DD hh:mm:ss a'), "WorkHours": this.getTimeShow(this.state.timeElapsed, this.state.startTime), "totaltime": this.state.totalworksec, "TaskStatus": jobStatus }
    const valueTask = { "Task_ID": "0", "Job_ID": jobId, AssignTo: assignToId, AssignBy: "0", "CreatedBy": "1", "Task_Status": "0" };
    this.props.dispatch({
  ...
  onStart = () => {
    if (this.state.networkAvailable) {
      ...
      this.setState({
        startTime: new Date(momentTz().tz('Asia/Kolkata').format()).getTime(),
        timerStart: true
      });

      this.interval = setInterval(() => {
        this.setState({
          timeElapsed: new Date(momentTz().tz('Asia/Kolkata').format()).getTime(),
          running: true
        });
      }, 1000);
    } else {
  ...
  onResume = () => {
    if (this.state.networkAvailable) {
      ...
      this.state.startTime = new Date(momentTz().tz('Asia/Kolkata').format()).getTime();
        ...
        <Grid>
          <Row>
            <Col style={{
              ...
              backgroundColor: this.state.taskstatus == "Paused" ? '#DCDCDC' : '#808080',
              marginRight: 1
            }} onPress={this.state.running ? this.onPause : this.onStart}

              disabled={this.state.taskstatus == "Paused"}
              // opacity={this.state.taskstatus == "Paused" ? 0.5 : 1}
              >

              <Icons name={(this.state.taskstatus == "Pending") ? 'play-circle-outline' : 'pause-circle-outline'} color="#fff" size={25} />
            ...
            <Col style={{
              ...
              // opacity={this.state.taskstatus != "Paused" ? 0.5 : 1}
              >
              <Icon name="ios-skip-forward" color="#fff" size={25} />
            ...
            <Col style={{
              ...
              justifyContent: 'center',
              paddingRight: 5,
              backgroundColor: this.state.taskstatus == "Pending" ? '#DCDCDC' : '#808080'
            }}
              onPress={() => this.onStop()}
              disabled={this.state.taskstatus == "Pending"}
              // opacity={this.state.taskstatus == "Pending" ? 0.5 : 1}
              >
              <FontAwesome name= "stop" color="#fff" size={20} />
              {/* <Icons name="stop-circle-outline" color="#fff" size={25} /> */}
