...
import { eventList, eventListJune, mainText, logo, myBreakTitle, wiredssid, defaultGatewayId } from '../../common/constant';
...
class MyBreak extends Component {
  ...
  componentWillMount() {
    NetInfo.getConnectionInfo().then((connectionInfo) => {
      if (connectionInfo.type == 'none') {
        ...
        const userValueID = this.props.userCredential.data.ID;
        const inputData = {
          "value": value,
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.myBreak !== this.props.myBreak) {
      this.setState({ listItem: nextProps.myBreak, loaderVisible: false })
      console.log("nextProps.myBreak", nextProps.myBreak);
    }
  }
  ...
  myJobHeader() {
    return myBreakTitle.map((data, i) => {
      return (
        // <TouchableOpacity style={styles.myJobId} key={i} onPress={() => this.onPressToSort(data.id)}>
        <Text style={{ color: '#fff', paddingTop: 10, paddingRight: 10 }}>{data.title}</Text>
        // </TouchableOpacity>
      )
    })
  }
  ...
  dataTable() {
    var breakList = this.props.myBreak;
    console.log("myJob$$dataTables", breakList);
    if (breakList !== undefined) {
      if (breakList.myBreak.Data !== undefined) {
        if (breakList.myBreak.Data.BreakList !== undefined && breakList.myBreak.Data.BreakList !== null) {
          return (
            <View style={{
              flex: 8,
              backgroundColor: "#fff"
              ...
              <ScrollView>
                {this.showTables(breakList.myBreak.Data.BreakList)}
              </ScrollView>
            </View>
          )
        } else {
    ...
    return dataValue.map((data, i) => {
      ...
      return (
        <Row key={i} style={[styles.myJobDataStyle, { backgroundColor: i % 2 !== 0 ? '#F5F5F5' : '#fff' }]} >
          <Col style={styles.textContainerStyle} size={35}>
            {/* <Text style={styles.tableContent}>{ !this.state.Job_ID ? i+1 : (dataValue.length)-i }</Text> */}
            <Text>{data.Name}</Text>
          </Col>
          <Col style={styles.textContainerStyle} size={45} >
            <Text>{data.Break_Name}</Text>
          </Col>
          <Col style={styles.textContainerStyle} size={35}>
            <Text>{data.A_by_name}</Text>
          </Col>
        </Row>
      )
    })
...
export default connect(mapStateToProps)(MyBreak);
