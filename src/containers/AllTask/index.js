...
import { eventList, eventListJune , mainText, logo, allTaskTitle, wiredssid, defaultGatewayId } from '../../common/constant';
...
class AllTask extends Component {
  ...
  componentWillMount() {
    ...
    const inputData =  {
        "Task_ID": "0", 
        "Job_ID": "0", 
        "AssignTo": this.props.userCredential.data.ID.toString(),
        "AssignBy": "0", "CreatedBy": "1", "Task_Status": "0" 
}
  ...
  componentWillReceiveProps(nextProps) {
    if(nextProps.allTask !== this.props.allTask) {
        this.setState({ dataAlltask: nextProps.allTask, loaderVisible: false})
        console.log("nextProps.allTask",nextProps.allTask);
      }
...
//   }
  ...
  myJobHeader() {
    return allTaskTitle.map((data, i) => {
      return (
        // <TouchableOpacity style={styles.myJobId} key={i} onPress={() => this.onPressToSort(data.id)}>
          <Text style={{color: '#fff' , paddingTop : 10, paddingRight : data.title == 'Total Time'|| data.title == 'Status' ? 24 : 15  }}>{data.title}</Text>
        // </TouchableOpacity>
      )
    })
  }
  ...
  dataTable() {
    var allTaskList = this.props.allTask;
    console.log(this.props.allTask);
    console.log(allTaskList);
    if(allTaskList !== undefined) {
      if(allTaskList.allTask.Data !== undefined) {
        if(allTaskList.allTask.Data.Tasklist !== undefined && allTaskList.allTask.Data.Tasklist !== null) {
            return (
              <View style={{flex: 8,
                backgroundColor: "#fff"}}>
                <ScrollView>
                  {this.showTables(allTaskList.allTask.Data.Tasklist)}
                </ScrollView>
              </View>
            )
        } else {
          return (
            <View style={{flex: 8, alignItems: 'center', padding: 20}}>
              <Text style={{color: '#d3d3d3'}}>No Task List</Text>
            </View>
          )
        }
      }
    ...
    return dataValue.map((data, i) => {
      ...
      return (
        <Row key={i} style={[styles.myJobDataStyle, {backgroundColor: i % 2 !== 0 ? '#F5F5F5' : '#fff'}]}>
          ...
          <Col style={styles.timeTextContainerStyle} size={25}>
          <Text>{data.Time}</Text>
          </Col>
